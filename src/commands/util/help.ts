...
import { findCommands, groupName, groups, loadedCommands, parseMessage } from '../../core/dispatcher'
...
async function usage(name: string, guildID?: string, usePref?: boolean) {
  const pref = !usePref ? '' :
    guildID ? (await getCommandPrefix()) : `@${client.user.tag} `
  return '`' + pref + name + '`'
}

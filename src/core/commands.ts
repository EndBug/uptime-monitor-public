...
import { isOwner, getSupportLink } from '../utils/utils'
import { owner, client } from '../core/app'
...
export class Command {
  ...
  async onError(err: Error, message: Message) {
    return message.reply(stripIndents`
			An error occurred while running the command: \`${err.name}: ${err.message}\`
			You shouldn't ever receive an error like this.
			Please contact ${owner.tag} in this server: ${await getSupportLink()}
		`)
  }
